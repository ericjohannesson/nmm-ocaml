...
let anon_arg_fun arg : unit =
  ...
  | 0 ->
      let _ : unit =
        ...
        | "exml-of-axml" ->
            keyspecdoc_list.contents <- exml_of_axml_list
        | "normalize-axml" | "validate-axml" | "validate-exml" ->
...
let _ : unit =
  ...
  | "validate-axml" -> (
      match read_from_stdin.contents with
      | true -> Main.validate_axml_file "-"
      | false -> (
          match path_to_xml_file.contents with
          | "" -> raise (Error "missing PATH-TO-AXML-FILE")
          | path -> Main.validate_axml_file path))
  | "validate-exml" -> (
      match read_from_stdin.contents with
      | true -> Main.validate_exml_file "-"
      | false -> (
          match path_to_xml_file.contents with
          | "" -> raise (Error "missing PATH-TO-EXML-FILE")
          | path -> Main.validate_exml_file path))
